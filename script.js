...
let favoriteFoods = ["Hamburger", "Pizza", "Sandwich", "Meatballs", "Hotdog", "Sushi", "Soup", "Fried Chicken"];
...
printFoodRecommendation("Hotdogs");
...
console.log("Foods with 'a' in the name:");
...
console.log("Foods with 'a' array:", foodsWithA);
...
for (let i = 0; i < friendFavorites.length; i++) {
  if (friendFavorites[i].length === longestLength) {
    longestFoods.push(friendFavorites[i]);
  }
}

// Adaption as there are two foods in the friendFavorites array with the same (highest) amount of characters
if (longestFoods.length === 1) {
  console.log(`The longest food name in the list is ${longestFoods[0]} with ${longestLength} characters.`);
} else {
  console.log(`The longest food names in the list are ${longestFoods.join(' and ')} with ${longestLength} characters each.`);
}